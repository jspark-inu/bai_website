...
이 계정은 등록 학생 목록에 포함되어 있습니다. 일반 UI·기능 PR은 GitHub의 `pytest`와
`react` 검사가 모두 통과하면 자동으로 squash merge되고 운영 사이트에 배포됩니다.
인증·DB·API·의존성·배포·GitHub 설정 같은 고위험 경로를 수정한 PR만 PI 검토를 기다립니다.
