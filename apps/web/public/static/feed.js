...
function feedSidebar(active, isPI) {
  ...
  const move = isPI ? '<div class="navsec">이동</div><a href="/cockpit" data-full="1">🛰 HAI OS</a><a href="http://100.96.96.101:8003/" target="_blank" rel="noopener">🎓 Professor OS</a>' : "";
  return `<aside class="side"><div class="brand">📰 BAI <span class="b">Feed</span></div>
    ...
    <div class="navsec">계정</div>${tab("/account?goodbai=1", "🔑 Goodbai API", "developer")}${tab("/account", "🔐 비밀번호 변경", "account")}${admin}
    ${move}</aside>`;
}
...
async function renderDeveloper(view) {
  view.innerHTML = `<div class="content">
    <h2 class="page-title">🔑 Goodbai API</h2>
    <p class="muted" style="margin-top:-8px">Codex /goodbai가 BAI 피드에 글을 올릴 때 쓰는 개인 API key입니다. 단체 채팅방이나 GitHub에 올리지 마세요.</p>
    <div class="editor">
...
async function renderAccount(view) {
  view.innerHTML = `<div class="content">
    ...
    <div class="editor">
      <div class="editor-head"><b>${esc(FEED_ME.name)} 계정</b><span>새 비밀번호는 8자 이상이어야 합니다.</span></div>
      <label>현재 비밀번호</label><input class="tags" id="currentPassword" type="password" autocomplete="current-password">
      <label>새 비밀번호</label><input class="tags" id="newPassword" type="password" autocomplete="new-password">
      <label>새 비밀번호 확인</label><input class="tags" id="confirmPassword" type="password" autocomplete="new-password">
      <div class="editor-actions"><p class="err" id="passwordErr"></p><button class="primary" id="changePasswordBtn" style="margin-left:0">변경</button><span class="muted" id="passwordMsg"></span></div>
    </div>
  </div>`;
  changePasswordBtn.onclick = async () => {
    passwordErr.textContent = ""; passwordMsg.textContent = "";
    const current = currentPassword.value;
    const next = newPassword.value;
    if (next !== confirmPassword.value) { passwordErr.textContent = "새 비밀번호 확인이 일치하지 않습니다."; return; }
    if (next.length < 8) { passwordErr.textContent = "새 비밀번호는 8자 이상이어야 합니다."; return; }
    ...
    if (r.ok) {
      ...
      passwordMsg.textContent = "변경됐습니다. 다음 로그인부터 새 비밀번호를 사용하세요.";
    } else if (r.status === 401) passwordErr.textContent = "현재 비밀번호가 맞지 않습니다.";
    else passwordErr.textContent = "변경하지 못했습니다.";
  };
}

async function renderAdminMembers(view) {
  if (FEED_ME.role !== "pi") {
    view.innerHTML = '<div class="content"><div class="empty-card">PI만 접근할 수 있습니다.</div></div>';
    return;
  }
  view.innerHTML = `<div class="content">
    <h2 class="page-title">🛡 멤버 관리</h2>
    <p class="muted" style="margin-top:-8px">학생이 비밀번호를 잊었을 때 새 비밀번호로 재설정합니다.</p>
    <div id="adminMembers"></div>
  </div>`;
  const rows = await (await fetch("/api/admin/members")).json();
  adminMembers.innerHTML = rows.map(m => {
    const meta = [m.role, m.status, m.last_post_at ? `최근 ${m.last_post_at.slice(0, 10)}` : "글 없음"].filter(Boolean).join(" · ");
    return `<div class="card admin-member" data-member="${m.id}">
      <div class="head">${avatar(m.name)}<span class="author">${esc(m.name)}</span><span>· ${esc(meta)}</span></div>
      <div class="comment-input password-reset">
        <input type="password" data-password-for="${m.id}" placeholder="새 비밀번호(8자 이상)" autocomplete="new-password">
        <button class="primary" data-reset-password="${m.id}" style="margin-left:0">재설정</button>
      </div>
      <p class="err" id="resetErr-${m.id}"></p><p class="muted" id="resetMsg-${m.id}"></p>
    </div>`;
  }).join("");
  adminMembers.querySelectorAll("[data-reset-password]").forEach(btn => btn.onclick = async () => {
    const mid = btn.dataset.resetPassword;
    const input = adminMembers.querySelector(`[data-password-for="${mid}"]`);
    const err = document.getElementById(`resetErr-${mid}`);
    const msg = document.getElementById(`resetMsg-${mid}`);
    err.textContent = ""; msg.textContent = "";
    const next = input.value;
    if (next.length < 8) { err.textContent = "새 비밀번호는 8자 이상이어야 합니다."; return; }
    const r = await fetch(`/api/admin/members/${mid}/password`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ new_password: next }),
    });
    if (r.ok) { input.value = ""; msg.textContent = "재설정됐습니다."; }
    else err.textContent = "재설정 실패";
  });
}
...
async function renderMaterials(view) {
  view.innerHTML = `<div class="content">
    ...
    <div class="editor hidden" id="materialEditor">
      <div class="editor-head"><b id="materialFormTitle">자료 올리기</b><span>파일, 링크, 본문 중 하나 이상을 올릴 수 있습니다.</span></div>
      ...
      <label>파일</label><input class="tags" id="materialFile" type="file">
  ...
  const resetForm = () => {
    ...
    materialGuild.value = ""; materialFile.value = ""; materialUrl.value = ""; materialBody.value = ""; materialErr.textContent = "";
  ...
  const materialCard = m => {
    ...
    return `<div class="card material-card" data-material="${m.id}">
      ...
      ${m.file_url ? `<div class="sec"><div class="label">파일</div><div class="body"><a class="linkchip" href="${esc(m.file_url)}">📎 ${esc(m.file_name || "첨부파일")}</a></div></div>` : ""}
  ...
  saveMaterialBtn.onclick = async () => {
    const selectedFile = materialFile.files && materialFile.files[0];
    const payload = new FormData();
    payload.append("title", materialTitle.value.trim());
    payload.append("category", materialCategory.value);
    payload.append("guild", materialGuild.value.trim());
    payload.append("url", materialUrl.value.trim());
    payload.append("body", materialBody.value.trim());
    if (selectedFile) payload.append("file", selectedFile);
    materialErr.textContent = "";
    if (!materialTitle.value.trim() || (!materialUrl.value.trim() && !materialBody.value.trim() && !selectedFile)) {
      materialErr.textContent = "제목과 파일, 링크, 본문 중 하나 이상을 입력해야 합니다.";
      return;
    }
...
function matchRoute(path) {
  ...
  if (pathname === "/account" && url.searchParams.get("goodbai") === "1") return ["developer", renderDeveloper];
...
const FEED_ROUTE_RE = /^\/(?:$|post\/|member\/|members|projects|materials|account|admin\/members|search|questions|ask|tag\/)/;
