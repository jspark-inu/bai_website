...
def create_verified_backup(db_path, backup_dir, stamp=None, keep=DEFAULT_KEEP):
    ...
    try:
        ...
        try:
            ...
            try:
                src.backup(bck)
                # A WAL-mode source can produce a WAL-mode backup image. On
                # macOS, opening that temporary hidden file with mode=ro can
                # fail because SQLite wants sidecar WAL/SHM handling even
                # though the copied data is complete. Publish backups in
                # rollback-journal mode so read-only verification and restore
                # checks work without writable sidecars.
                bck.execute("PRAGMA journal_mode=DELETE")
            finally:
                bck.close()
        finally:
            src.close()
